...
export function TradeAppsSyncSection({ project, projectId, embedded }: Props) {
  ...
  async function onToggleVisibility(nextHidden: boolean) {
    ...
    try {
      ...
      setStatus(
        nextHidden
          ? "Project hidden from Field Tools, Field View, digest emails, order history, and Manpower Cal."
          : "Project visible in Field Tools, Field View, digest emails, order history, and Manpower Cal.",
      );
    } catch (e) {
  ...
  const body = (
    <>
      ...
      <label className="checkbox-row paint-tracker-flag">
        ...
        />
        <span>
          Hide from Field Tools ordering, Field View, digest emails, order history, and Manpower Cal
