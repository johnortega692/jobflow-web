...
import { fillLegacyXlsInPlace } from "./xlsSurgicalFill";
...
export function isLegacyXlsFile(bytes: Uint8Array): boolean {
  return bytes.length >= OLE2_SIGNATURE.length && OLE2_SIGNATURE.every((b, i) => bytes[i] === b);
}
...
export async function patchXlsxCellValues(
  ...
  if (isLegacyXlsFile(new Uint8Array(templateBytes.slice(0, 8)))) {
    return fillLegacyXlsInPlace(templateBytes, cellValues);
  }
