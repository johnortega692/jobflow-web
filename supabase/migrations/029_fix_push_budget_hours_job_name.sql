-- Fix ambiguous column reference: PL/pgSQL variable job_name vs manpower_hours_jobs.job_name
...
begin
  ...
  v_job_name := trim(coalesce(proj.job_number, '') || ' ' || coalesce(proj.job_name, ''));
  if v_job_name = '' then
